(function() {
    ...
    Game1.prototype.setColor = function(row, col, num) {
        // 给对应的有颜色的方块添加类名
        $(".tab1").find("tr").eq(row).children("td").eq(col).addClass("c" + num)
    };
    Game1.prototype.setNextColor = function(row, col, num) {
        // 给对应的有颜色的方块添加类名
        for (var i = 0; i < 4; i++) {
            for (var j = 0; j < 4; j++) {
    ...
    // 清屏功能
    Game1.prototype.clear = function() {
        for (var i = 0; i < this.row; i++) {
            for (var j = 0; j < this.col; j++) {
    ...
    Game1.prototype.bindEvent = function() {
        // 备份
        var self = this;
